...
int main() {
    try {
        std::cout << "Creating timed door with 2 second timeout..."
                  << std::endl;
        TimedDoor door(2);
        Timer timer;

        std::cout << "Door is initially closed: "
                  << (door.isDoorOpened() ? "open" : "closed")
                  << std::endl;
        ...
        std::cout << "Starting timer for " << door.getTimeOut()
                  << " seconds..." << std::endl;
